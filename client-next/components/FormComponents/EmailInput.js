...
const EmailInput = ({ email, setEmail }) => {
    ...
    const emailVerification = () => {
        if (emailFound === 1 && EMAIL_REGEX.test(email)) return 0;
        if (emailFound === 0 && EMAIL_REGEX.test(email)) return 1;
        return 0;
    };
        ...
        <div className={`flex flex-col gap-2 w-full`}>
            ...
            <div className={`flex-col gap-1 pt-2 pb-2 ${isInputFocused ? "flex" : "hidden"}`}>
                <ReqLabel
                    ...
                    req={`${EMAIL_REGEX.test(email) && emailFound === 0 ? "Looks Good" : EMAIL_REGEX.test(email) && emailFound === 1 ? "Email already registered" : "Enter proper Email"}`}
